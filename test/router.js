...
describe('router:URLFromRoute', () => {
  ...
  it('serializes query string', () => {
    assert.equal(URLFromRoute({ screen: '', search: { abc: '123' } }),
                 '/?abc=123');
  });
  ...
  it('serializes % encoded query string parameters in keys and values', () => {
    assert.equal(URLFromRoute({ screen: '', search: { 出身: '仙台' } }),
                 '/?%E5%87%BA%E8%BA%AB=%E4%BB%99%E5%8F%B0');
  });

  it('serializes % encoded emoji', () => {
    assert.equal(URLFromRoute({ screen: '', search: { feeling: '😊' } }),
                 '/?feeling=%F0%9F%98%8A');
  });

  it('serializes + encoded query string parameters', () => {
    assert.equal(URLFromRoute({ screen: '', search: { abc: 'foo hoge' } }),
                 '/?abc=foo+hoge');
  });
  ...
  it('serializes % encoded fragments', () => {
    assert.equal(URLFromRoute({ screen: '', fragment: 'one two' }),
                 '/#one%20two');
  });
...
describe('router:routesEqual', () => {
  it('compares equal routes', () => {
    ...
    for (const route of testRoutes) {
      assert.isTrue(routesEqual(routeFromURL(route), routeFromURL(route)),
    ...
    for (let i = 0; i < testRoutes.length; i++) {
      ...
      assert.isFalse(routesEqual(routeFromURL(routeA), routeFromURL(routeB)),
                     `Routes from '${routeA}' and '${routeB}' should NOT`
                     + ' compare equal');
    }
  });
});
